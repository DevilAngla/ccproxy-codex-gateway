...
class CodexAdapter(BaseHTTPAdapter):
    ...
    def _sanitize_provider_body(self, body_data: dict[str, Any]) -> dict[str, Any]:
        ...
        list_input = self._normalize_responses_input(body_data.get("input", []))
    ...
    def _normalize_responses_input(self, raw_input: Any) -> list[dict[str, Any]]:
        """Normalize OpenAI Responses-style input into Codex message objects.

        The Responses API allows `input` to be sent as a plain string. Codex's
        provider adapter expects a list of structured input items, so we coerce
        the simple string form into a standard single user message.
        """

        if isinstance(raw_input, str):
            return [self._build_text_message_input(raw_input)]
        ...
        for item in raw_input:
            if isinstance(item, str):
                normalized_input.append(self._build_text_message_input(item))
    ...
    @staticmethod
    def _build_text_message_input(text: str) -> dict[str, Any]:
        """Wrap a plain text prompt as a single user message."""
