...
class TestCodexAdapter:
    ...
    @pytest.mark.asyncio
    async def test_prepare_provider_request_wraps_string_input_for_responses(
        self, adapter: CodexAdapter
    ) -> None:
        """Plain-string Responses input should be normalized for Codex."""
        body = json.dumps(
            {
                "model": "gpt-5.4",
